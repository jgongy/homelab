...
data "talos_client_configuration" "this" {
  ...
  nodes                = [for k, v in var.nodes : (v.internal_ip == null ? v.external_ip : v.internal_ip)]
...
data "talos_machine_configuration" "this" {
  for_each         = var.nodes
  cluster_name     = var.cluster.name
  cluster_endpoint = "https://${var.cluster.endpoint != null ? var.cluster.endpoint : [for k, v in var.nodes : v.external_ip if v.k8s_node_type == "controlplane"][0]}:6443"
  talos_version    = var.cluster.talos_version
  ...
  config_patches   = [
    templatefile(
      ...
      merge(
        {
          hostname = each.key
          internal_subnet  = var.cluster.internal_subnet == null ? "" : var.cluster.internal_subnet
        ...
        each.value.k8s_node_type == "controlplane" ? {
          ...
          pod_cidr_subnet  = var.cluster.pod_cidr_subnet
          vip              = var.cluster.endpoint == null ? "" : var.cluster.endpoint
...
resource "talos_machine_configuration_apply" "this" {
  ...
  for_each                    = var.nodes
  node                        = each.value.internal_ip == null ? each.value.external_ip : each.value.internal_ip
...
resource "talos_machine_bootstrap" "this" {
  node                 = [for k, v in var.nodes : (v.internal_ip == null ? v.external_ip : v.internal_ip) if v.k8s_node_type == "controlplane"][0]
...
data "talos_cluster_health" "this" {
  ...
  control_plane_nodes  = [for k, v in var.nodes : (v.internal_ip == null ? v.external_ip : v.internal_ip) if v.k8s_node_type == "controlplane"]
  worker_nodes         = [for k, v in var.nodes : (v.internal_ip == null ? v.external_ip : v.internal_ip) if v.k8s_node_type == "worker"]
...
resource "talos_cluster_kubeconfig" "this" {
  ...
  node                 = [for k, v in var.nodes : (v.internal_ip == null ? v.external_ip : v.internal_ip) if v.k8s_node_type == "controlplane"][0]
  endpoint             = var.cluster.endpoint != null ? var.cluster.endpoint : [for k, v in var.nodes : v.external_ip if v.k8s_node_type == "controlplane"][0]
  client_configuration = talos_machine_secrets.this.client_configuration
  timeouts = {
    read = "1m"
  }
